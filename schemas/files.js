NEWSCHEMA('Files', function (schema) {
	...
	schema.action('read', {
		...
		action: function ($) {
			var params = $.params;
			DB().one('nosql/files').error('@(File not found)').where('id', params.id).callback($.callback);
		}
	});
	...
	schema.action('create', {
		action: function ($, model) {
			...
			model.dtcreated = NOW;
			DB().insert('nosql/files', model).callback((error, res) => {

				F.Fs.readFile(file.path, function (err, data) {
					if (err) { $.invalid(err); return; }
					F.Fs.writeFile(PATH.public(`/downloads/${file.filename}`), data, function () {
						$.success(model.id);
					})
				});
			});
	...
	schema.action('remove', {
		...
		action: async function ($) {
			var params = $.params;
			DB().one('nosql/files').error('@(File not found)').where('id', params.id).callback((error, response) => {
